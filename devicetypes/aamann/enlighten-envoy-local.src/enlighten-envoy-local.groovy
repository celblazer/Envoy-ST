...
def parse(String message) {
		...
		if (msg.body ==~ /${patternString}/) {
			msg.body.replaceFirst(/${patternString}/) {all, dateString ->
				try {
                    ...
                    state.installationDate = new Date().parse("MMM dd, yyyy H:m a z", dateString).getTime()
	...
	def events = []
	events << createEvent(name: 'power_details', value: ("(" + String.format("%+,d", powerChange) + "W) Peak Today: " + String.format("%,d", state.peakpower) + "W (" + String.format("%.1f", state.peakpercentage) + "%)"), displayed: false)
	...
	if (state.installationDate && state.installationDate > 0) {
		...
		events << createEvent(name: 'efficiency_lifetime', value: String.format("%#.3f", efficiencyLifetime), displayed: false)
	}
...
def parse(String message) {
	...
	if (!state.installationDate && !msg.json && msg.body) {
		...
		if (msg.body ==~ /${patternString}/) {
			...
			log.debug "${device.displayName} - unable to find installation date on page. Please enter manually in settings"
			//state.installationDate = -1
            state.installationDate = new Date().parse("MMM dd, yyyy", settings.datedata).getTime()
            sendEvent(name: 'installationDate', value: "System live since " + new Date(state.installationDate).format("MMM dd, yyyy"), displayed: false)
		}
